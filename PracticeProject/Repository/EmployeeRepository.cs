...
namespace PracticeProject.Repository
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public EmployeeRepository(IConfiguration configuration) // Configure DB connection
        {
        ...
        public Employee Find(int id)
        {
            var sql = "SELECT * FROM Employees " +
                "WHERE EmployeeId = @Id";
            return db.Query<Employee>(sql, new { @Id = id }).Single(); // Ensures we only have one ID
        ...
        public Employee Update(Employee employee)
        {
            var sql = "UPDATE Employees " +
                "SET Name = @Name, Title = @Title, Email = @Email, Phone = @Phone, CompanyId = @CompanyId " +
